"""SQLite repository for durable spool->MP4 encode job tracking."""
...
class EncodeJobsRepo:
        ...
        with sqlite3.connect(self.db_path) as conn:
            conn.execute(
                ...
                INSERT INTO encode_jobs (
                    segment_base, camera_serial, raw_path, output_mp4,
                    width, height, fps, pixel_format, status, created_at, updated_at
                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'pending', ?, ?)
                """,
                (segment_base, camera_serial, raw_path, output_mp4, width, height, float(fps), pixel_format, now, now),
            )
            conn.commit()
    ...
    def claim_next_job(conn: sqlite3.Connection, worker_id: str) -> EncodeJob | None:
        ...
        row = conn.execute("""
            SELECT id, segment_base, camera_serial, raw_path, output_mp4,
